...
public class FlightsAdapter extends RecyclerView.Adapter<FlightsAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.img.setImageURI(item.getImages());
        holder.land_place.setText(item.getDestination());
        holder.leave_place.setText(item.getOrigin());
        holder.time_to_land.setText(formatter.format(new Date(Long.parseLong(item.getArrival_time()))));
        holder.time_to_leave.setText(formatter.format(new Date(Long.parseLong(item.getDeparture_time()))));
